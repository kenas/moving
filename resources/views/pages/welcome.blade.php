	...
	@if($articles)
		...
		@foreach($articles as $article)
			...
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				...
				<i class="far fa-clock"></i> <small class="text-muted">{{date("d F Y, g:i a", strtotime($article->created_at))}} | {{$article->created_at->diffForHumans()}} | autor: {{$article->author}} | Kategorie </small> <a href="{{ route('category.index', strtolower($article->category->name))}}"><span class="badge badge-info">{{$article->category->name}}</span></a>
